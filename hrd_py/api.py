...
class HRDApi:
    ...
    def partner_get_balance(self) -> Dict[str, float]:
        ...
        if balance_elem is not None:
            return {
                "balance": float(balance_elem.find("balance").text),
                "restricted_balance": float(balance_elem.find("restrictedBalance").text)
            }
    ...
    def domain_renew(self, domain_name: str, current_expiry_date: str, period: int = 1) -> int:
        params = {
            "name": domain_name,
            "currentExpirationDate": current_expiry_date,
            "period": period
        }
